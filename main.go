...
func init() {
	...
	var help = `
gogloo ` + Version + `
Test the latency and speed of all gogloo to obtain the lowest latency and port.
...
`
	...
	flag.IntVar(&task.Routines, "n", 1, "Latency test threads")
	flag.IntVar(&task.PingTimes, "t", 1, "Number of latency test times")

	flag.IntVar(&maxDelay, "tl", 3000, "Average latency upper limit")
	...
	flag.IntVar(&utils.PrintNum, "p", 0, "Number of results to display")
	flag.StringVar(&task.IPFile, "f", "", "IP segment data file")
	flag.StringVar(&task.IPText, "ip", "", "Specify IP segment data")
	flag.StringVar(&utils.Output, "o", "1.tmp", "Output result file")
